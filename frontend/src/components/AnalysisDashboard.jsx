...
import { MOCK_PR_DATA, computeProbability, getAIExplanation } from '../data/mockData';
...
export default function AnalysisDashboard({ prUrl }) {
    const data = { ...MOCK_PR_DATA, url: prUrl };
    ...
    const togglesForCalc = {
        addTests: activeToggles.add_tests,
        improveDesc: activeToggles.improve_desc,
        reduceSize: activeToggles.reduce_size,
    };

    const currentScore = computeProbability(data.baseProbability, togglesForCalc);
    const explanation = getAIExplanation(currentScore, data);

    const pulsingMap = { add_tests: 'tests', improve_desc: 'desc', reduce_size: 'loc' };
    const pulsingFactors = Object.entries(activeToggles)
        .filter(([, v]) => v)
        .map(([k]) => pulsingMap[k])
        .filter(Boolean);
    ...
    return (
        <motion.div
            ...
            className="w-full max-w-6xl mx-auto px-4 pb-16 flex flex-col gap-4"
        >
            {/* PR badge */}
            <motion.div
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 self-center"
            >
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" className="text-neon-green flex-shrink-0">
                ...
                <span className="text-white/50 text-xs font-mono truncate max-w-sm">{prUrl || data.repo}</span>
            </motion.div>
            ...
            <motion.div
                ...
                style={{ display: 'grid', gridTemplateColumns: '1fr 288px', gap: '1rem', alignItems: 'stretch' }}
            >
                {/* Tabbed panel */}
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                {activeTab === 'risk' && (
                                    <motion.div
                                        ...
                                        className="p-5"
                                    >
                                        <RiskBreakdownInline factors={data.riskFactors} pulsingFactors={pulsingFactors} />
                                    </motion.div>
                                )}
...
function PRSummaryInline({ data }) {
    ...
    return (
        <div>
            <h3 className="text-white font-semibold text-sm leading-snug mb-4 line-clamp-2">{data.title}</h3>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
                {stats.map((s, i) => (
                    <motion.div key={s.label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.07 }}
                        ...
                        <div className={`text-xl font-bold font-mono ${s.color} leading-none mb-1`}>{s.value}</div>
                ...
                <div className="flex gap-1.5">
                    {data.reviewers.map((r, i) => (
                        <motion.div key={r} initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.3 + i * 0.07 }}
                            className="w-6 h-6 rounded-full bg-surface-600 border border-white/10 flex items-center justify-center text-[9px] text-white/60 font-bold uppercase" title={r}>
                            {r.slice(0, 2)}
                        </motion.div>
                    ))}
                </div>
    ...
    return (
        <div className="flex flex-col gap-2">
            <div className="flex items-center justify-between mb-1">
                <span className="text-white/30 text-xs">{factors.length} risk factors detected</span>
            </div>
